...
;;; NOTICE:
;;;
;;; This version of pod-mode.el has been modified by Kevin C. Krinke for the
;;; quo-emacs project.  The following is the list of changes made:
;;;
;;;  * require `cl-lib' instead of the deprecated `cl'
;;;  * replaced all instances of `loop' with `cl-loop'
;;;  * re-indented the entire file (blank space changes)
...
(defvar pod-version "1.04"
  "Version of POD mode.")

(let* ((head-sizes '(1.9 1.7 1.5 1.3))
       (heads (cl-loop for i from 1 to (length head-sizes) collect
                       (cons i (nth (- i 1) head-sizes)))))
  (defconst pod-font-lock-keywords-1
    (append
     (cl-loop for (n . s) in heads collect
       ...
       (let ((beg (or (match-end 1)
         ...
         (when (re-search-forward
                (concat (when (> n-lt 1) "\s")
                        "\\("
                        (apply 'concat (cl-loop for i from 1 to n-lt collect ">"))
                        "\\)")
                limit t)
...
the POD defaults."
  ...
  (setq outline-regexp
        (format "=%s\s"
                (regexp-opt
                 (append (cl-loop for i from 1 to 4 collect (format "head%d" i))
                         '("item") sections))))
...
the POD defaults."
  (setq imenu-generic-expression
        `((nil ,(format "^=\\(?:%s\\)\s+\\(.*\\)"
                        (regexp-opt
                         (append
                          (cl-loop for i from 1 to 4 collect (format "head%d" i))
                          '("item") sections)))
               1))))
